		...
		Bukkit.getScheduler().runTaskAsynchronously(Main.getInstance(), () -> {
			try {
				...
				this.homes.forEach((name, location) -> {
					try {
						PreparedStatement statement = Main.getDatabaseManager().getConnection().prepareStatement("INSERT INTO `user_homes` (`uniqueId`, " +
								"`name`, `location`)" + "VALUES(?, ?, ?) ON " + "DUPLICATE KEY UPDATE `uniqueId` = VALUES(`uniqueId`), `name` = VALUES(`name`)"
