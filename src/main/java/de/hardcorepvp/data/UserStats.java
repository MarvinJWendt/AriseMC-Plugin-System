...
public class UserStats extends DatabaseLoader {
		...
		Bukkit.getScheduler().runTaskAsynchronously(Main.getInstance(), () -> {
			try {
				PreparedStatement statement = Main.getDatabaseManager().getConnection().prepareStatement(
						"INSERT INTO `user_stats` (`uniqueId`, `kills`, `deaths`)" +
	...
	public void readFromDatabase() {
		Bukkit.getScheduler().runTaskAsynchronously(Main.getInstance(), () -> {
			try {
				PreparedStatement statement = Main.getDatabaseManager().getConnection().prepareStatement("SELECT `kills`, `deaths` FROM `user_stats` WHERE `uniqueId` = ?");
				statement.setString(1, this.uniqueId.toString());
